...
| `go-obs.metrics.high-cardinality` | High | Proven Prometheus vector label names, label maps, or `WithLabelValues` arguments use request- or user-specific dimensions |
...
The high-cardinality check does not classify arbitrary slices or maps by vocabulary. Label-name findings require an exact, unshadowed `prometheus` or `promauto` import and a vector constructor whose second argument is either a direct `[]string` literal or one uniquely visible, unreassigned slice definition. Label-map findings require the imported `prometheus.Labels` type, while value findings require the actual `WithLabelValues(...)` call shape and a syntactically unbounded request URL, user ID, request ID, or error string rather than a bounded string literal. Modified files report only when that semantic metric relationship is new and a changed constructor, import, label, map key, or value supplies a current anchor. Ordinary documentation slices, fake constructors, unrelated `Labels` types, shadowed imports, unchanged legacy metrics, unrelated edits, tests, parse failures, and comment-only edits stay quiet.
