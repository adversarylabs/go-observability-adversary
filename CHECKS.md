...
| **Looks for** | Same-file direct calls where a callee's explicit `ctx.Err()` or `errors.Is(..., context.Canceled/DeadlineExceeded)` branch suppresses lifecycle effects or emits debug/info shutdown telemetry, returns the same error, and the caller unconditionally passes that result to `Error`/`Errorf`/`Errorw` |
| **Stays quiet when** | The caller filters cancellation; only retries, reconnects, or propagates; the callee treats cancellation as exceptional; or resolution would require interfaces, cross-file types, or multiple call hops |
